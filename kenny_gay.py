...
# ---------------- Admin Commands ---------------- #
# ---------------- Safari stop helper ---------------- #
async def stop_safari_for_account(user_id: int, account_name: str) -> str:
    """
    Stop safari hunting for a specific account.
    Cleans up watchdogs, background task and disconnects the Telethon client.
    """
    account_key = f"{user_id}_{account_name}"
    info = safari_status.get(account_key)

    if not info or not info.get("running"):
        return f"❌ {account_name} is not running."

    # mark not running so handlers immediately ignore further events
    info['running'] = False

    # cancel watchdog if present
    watchdog = info.get('hunt_watchdog')
    try:
        if watchdog and not watchdog.done():
            watchdog.cancel()
    except Exception as e:
        print(f"❌ {account_key}: Error cancelling watchdog - {e}")

    # cancel background run task if present
    task = info.get('task')
    try:
        if task and not task.done():
            task.cancel()
    except Exception as e:
        print(f"❌ {account_key}: Error cancelling task - {e}")
    ...
    try:
        if client:
            await client.disconnect()
            # small sleep to let Telethon cleanup sockets
            await asyncio.sleep(0.2)
    except Exception as e:
        print(f"❌ {account_key}: Error during client disconnect - {e}")

    # remove from safari_status
    safari_status.pop(account_key, None)

    return f"🛑 Stopped safari for {account_name}!"

# ---------------- Admin Commands (fixed) ---------------- #

@authorized_only
async def start_safari(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Start safari for all accounts concurrently."""
    user_id = update.effective_user.id
    ...
    for col_name in db.list_collection_names():
        if not col_name.startswith("user_"):
            continue
        try:
            uid = int(col_name.split("_", 1)[1])
        except Exception:
            continue
        ...
        try:
            ...
            print(f"⚠️ Failed to fetch accounts from {col_name}: {e}")
            continue

        for account in accounts:
            acc_name = account.get("account")
            session = account.get("session")
            if not acc_name or not session:
                continue
            tasks.append(start_safari_for_account(uid, acc_name, session))

    results = await asyncio.gather(*tasks, return_exceptions=True)
    started_count = sum(1 for r in results if isinstance(r, str) and (r.startswith("✅") or "Started" in r))
...
async def stop_safari(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Stop safari for all accounts concurrently."""
    user_id = update.effective_user.id
    if not is_owner(user_id):
        await update.message.reply_text("❌ Only the owner can use this command.")
        return
    ...
    for col_name in db.list_collection_names():
        ...
        for account in accounts:
            ...
            if not acc_name:
                continue
            tasks.append(stop_safari_for_account(uid, acc_name))

    results = await asyncio.gather(*tasks, return_exceptions=True)
    stopped_count = sum(1 for r in results if isinstance(r, str) and (r.startswith("🛑") or "Stopped" in r))
...
async def solo_stop_safari(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    This searches across all user_* collections and stops the first matching account.
    Usage: /solo_stop_safari <account_number_or_phone>
    """
    user_id = update.effective_user.id
    if not is_owner(user_id):
        await update.message.reply_text("❌ Only the owner can use this command.")
    ...
    target = context.args[0]
    found = False
    # search all user_* collections for matching account or phone
    for col_name in db.list_collection_names():
        if not col_name.startswith("user_"):
            continue
        try:
        ...
        try:
            ...
            print(f"⚠️ Failed searching in {col_name}: {e}")
            continue
        ...
        if account:
            ...
            if not acc_name:
                continue
            try:
                res = await stop_safari_for_account(uid, acc_name)
                await update.message.reply_text(res)
            except Exception as e:
                await update.message.reply_text(f"❌ Error stopping {acc_name}: {e}")
            found = True
            break
...
async def run_safari_hunt(user_id: int, account_name: str, client: TelegramClient, safari_bot):
    ...
    try:
        ...
        while True:
            ...
            if "entry fee deducted" in text or "you are already in the" in text or "welcome" in text:
                entered = True
                break

            # If nothing matched → retry enter
            await client.send_message(safari_bot, "/enter")

        if not entered:
            print(f"❌ {account_name}: Could not confirm safari entry")
            return

        # ✅ ADD THIS: Send notification that we entered safari zone successfully
        await notify_safari_status(user_id, account_name, "💥  Entered safari zone and started hunt successfully")
...
def setup_safari_handlers(client: TelegramClient, user_id: int, account_name: str, safari_bot):
    ...
    async def safari_message_edited_handler(event):
        ...
        # **FIXED: Safari ball failed - retry throw ball**
        if "safari ball failed" in text.lower():
            try:
                await message.click(text="Throw ball")
                await zzz(0.5)
                await message.click(text="Throw ball")
        ...
        if any(word in text_lower for word in ["caught", "fled", "fainted", "tm", "an expert trainer"]):
            print(f"🔄 {account_name}: Hunt completed - sending next hunt")
            await asyncio.sleep(7)  # Random delay like ref_deep.py
            
            if safari_status.get(account_key, {}).get('running'):
                await send_safari_hunt(client, account_key, safari_bot)
    
async def notify_safari_status(user_id: int, account_name: str, text: str):
    account_key = f"{user_id}_{account_name}"
    status = safari_status.get(account_key)
    if not status:
        return

    client: TelegramClient = status.get('client')
    col = user_collection(user_id)
    acc = col.find_one({"account": account_name})
    notify_chat_id = acc.get("NOTIFY_CHAT_ID", NOTIFY_CHAT_ID)

    try:
        await client.send_message(notify_chat_id, f"[{account_name}] {text}")
    except Exception as e:
        print(f"❌ {account_name}: Failed to notify safari status - {e}")
